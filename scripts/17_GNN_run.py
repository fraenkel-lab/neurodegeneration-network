...
def run_model(in_graph,device,dim,epochs=100,lr=0.01,weight_decay=5e-3):
    ...
    for epoch in t:
        ...
        loss = F.mse_loss(log_logits, in_graph.y)
        loss.backward()
        optimizer.step()
        ...
        t.set_description('[Train_loss:{:.6f}'.format(train_loss))
...
def main():
    ...
    ## create random values with the number of non-zero nodes, randomly assigned
    y=torch.zeros(len(node_df.index),dtype=torch.float32)
    n_shuffle=len(node_df[node_df['source'].isin(['None'])].index)
    rand_idx=torch.randperm(y.size()[0])[0:n_shuffle]
    y[rand_idx]=torch.randn(n_shuffle)
    y=y.reshape(len(node_df.index),1)
    ...
    #shuffled_idx=torch.randperm(y.size()[0])
    #y=y.view(-1)[shuffled_idx].view(y.size())

    # make torch dat
    in_graph=Data(x=x,edge_attr=edge_attr,edge_index=edge_index,y=y,num_classes=1)

    ## after grid search params: epochs=100, lr=0.01, weight decay 1e-4
    epochs = 100
    lr = 1e-2
    weight_decay = 1e-4
    dim = 16
    ...
    model=run_model(in_graph,device,dim,epochs,lr,weight_decay)
    ...
    explanation = explainer(
        x=in_graph.x,
        edge_index=in_graph.edge_index
    )


    print(f'Generated model explanations in {explanation.available_explanations}')

    torch.save(explanation, 'regression_explanation_model_GCN_randomized_randval.pt')
